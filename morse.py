...
def convert_morse():
    ...
    for character in message_input:
        if character in morse_rules:
            morse_text += morse_rules.get(character) + ' '
        else:
            print(
                f'El caracter {character} no se pudo convertir a Morse, se mostrará {character}')
...
def morse_translator():
    ...
    for morse in morse_characters:
        if morse in translator_rules:
            message.append(translator_rules.get(morse))
        else:
            print(
                f'El caracter {morse} no está en Morse, se imprimirá {morse}')
